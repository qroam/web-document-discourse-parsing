...
class PairBERTEncoder(nn.Module):
    def __init__(self, model_name_or_path='bert-base-chinese', config=None, gradient_checkpointing=True):
        ...
        print("initialized PairBERTEncoder")
        print(f"config.gradient_checkpointing = {self.sentence_bert.config.gradient_checkpointing}")

    # def forward(self, inputs):
    def forward(self, input_ids, token_type_ids, attention_mask=None, **kwargs):
        """
        ...
        token_type_ids = torch.reshape(token_type_ids, (-1, max_seq_length))

        outputs = self.sentence_bert(input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask,)
