...
class PairwisePointerNetwork(nn.Module):
    ...
    def forward(self, paragraphs, padding=None, golden=None):
        ...
        '''if self.layer_type=="bilinear":
            ...
            # New implementation:
            father_node_logit_matrix = torch.zeros(batch_size, num_paragraphs, num_paragraphs).to(paragraphs.device)
            # lstm_hidden = torch.zeros(self.max_paragraph_num).to(paragraphs.device)  # TODO: 0721
            # lstm_cell = torch.zeros(self.max_paragraph_num).to(paragraphs.device)  # TODO: 0721
            lstm_hidden = torch.zeros(self.input_dims).to(paragraphs.device)  # TODO: 0801
            lstm_cell = torch.zeros(self.input_dims).to(paragraphs.device)  # TODO: 0801
            for t in range(num_paragraphs):
                ...
                relative_position_encodings = self.get_relative_position_encoding_for_tail(self, tail_vectors, tail_index=t)


                tails = torch.cat((tails, relative_position_encodings), -1)
                # tails = torch.cat((tails, lstm_hidden.repeat(1, num_paragraphs, 1)), -1)  # TODO: 0721

                father_node_logit_matrix[:,t,:] = self.bilinear(head_vectors, tails).permute(0,2,1)  # (Batch, num_paragraph, 1) -> (Batch, 1, num_paragraph)
                
                interact = torch.cat((head_vectors, lstm_hidden.repeat(1, num_paragraphs, 1)),dim=2)
                
                probability_scores = self.score_mlp(interact)
                probability_scores = probability_scores.permute(0,2,1)  # TODO: 0801
                
                father_node_logit_matrix[:,t,:] = father_node_logit_matrix[:,t,:].clone() * probability_scores  # TODO: 0801
                father_node_predict = torch.argmax(father_node_logit_matrix[:,t,:t]) if t > 0 else 0 # TODO: 0721
                # lstm_cell, lstm_hidden = self.iterlstm(head_vectors[0,father_node_predict,:], lstm_cell, lstm_hidden)  # TODO: 0721
                # lstm_cell, lstm_hidden = self.iterlstm(self.position_encoding(torch.tensor(father_node_predict).to(paragraphs.device)), lstm_cell, lstm_hidden)  # TODO: 0722
                lstm_cell, lstm_hidden = self.iterlstm(head_vectors[:,father_node_predict,:], lstm_cell, lstm_hidden)  # TODO: 0801
            attn_mask = torch.triu(torch.ones(num_paragraphs, num_paragraphs)).to(paragraphs.device)
            father_node_logit_matrix = father_node_logit_matrix * (1 - attn_mask)
            
        
        elif self.layer_type=="new":
            father_node_logit_matrix = torch.zeros(batch_size, num_paragraphs, num_paragraphs).to(paragraphs.device)
            lstm_hidden = torch.zeros(paragraphs.shape[2]).to(paragraphs.device)  # TODO: 0721
            lstm_cell = torch.zeros(paragraphs.shape[2]).to(paragraphs.device)  # TODO: 0721
            for t in range(num_paragraphs):
                tails = tail_vectors[:,t,:].repeat(1, num_paragraphs, 1)  # (Batch, num_paragraph, hidden_dim)
                ...
                medim = self.activate(self.bilinear(head_vectors, tails))  # (Batch, num_paragraph, 64)
                # print(medim.shape)
                lstm_hidden_repeat = lstm_hidden.repeat(1, num_paragraphs, 1).reshape(1,num_paragraphs, -1)
                # print(lstm_hidden_repeat.shape)
                # print(relative_position_encodings.shape)
                father_node_logit_matrix[:,t,:] = self.linear(torch.cat((medim, relative_position_encodings, lstm_hidden_repeat), -1)).permute(0,2,1)  # (Batch, num_paragraph, 1) -> (Batch, 1, num_paragraph)
                father_node_predict = torch.argmax(father_node_logit_matrix[:,t,:t]) if t > 0 else 0 # TODO: 0721
                lstm_cell, lstm_hidden = self.iterlstm(head_vectors[0,father_node_predict,:], lstm_cell, lstm_hidden)  # TODO: 0721
            attn_mask = torch.triu(torch.ones(num_paragraphs, num_paragraphs)).to(paragraphs.device)
            father_node_logit_matrix = father_node_logit_matrix * (1 - attn_mask)
...
'''
        ...
        if golden is not None:
            ...
            flatten_father_node_logit_matrix = torch.flatten(father_node_logit_matrix[:, 1:, :], end_dim=1)
